...
def test_end_to_end():
    """端到端测试：合成数据 -> per-run micro 统计 -> 聚合 -> 跨样本汇总。"""
    ...
    try:
        ...
        for run_id in ["run_20240601_120000", "run_20240601_130000"]:
            ...
            # 5 段：
            #   0: radius=1.0 (< 2.5, 3 nodes) -> micro
            #   1: radius=3.0 (>= 2.5)         -> NOT micro
            #   2: radius=1.5 (< 2.5, 3 nodes) -> micro
            #   3: radius=0.5 (< 2.5, 2 nodes) -> NOT micro (nodes < 3)
            #   4: radius=2.0 (< 2.5, 3 nodes) -> micro
            ...
            for suffix, data in [
                ...
                with open(
                    os.path.join(run_dir, f"generate_vessel_{suffix}.txt"), "w"
                ) as f:
                    for v in data:
                        f.write(f"{v}\n")
        ...
        # ---- Test 1: write_micro_stats (per-run) ----
        for run_id in ["run_20240601_120000", "run_20240601_130000"]:
            run_dir = os.path.join(sample_dir, run_id)
            result = write_micro_stats(run_dir, 0.078)
            assert result is not None, f"write_micro_stats returned None for {run_id}"
            assert os.path.exists(
                os.path.join(run_dir, "statistics_summary_micro.txt")
            ), "Missing statistics_summary_micro.txt"

        print("PASS: test_write_micro_stats")

        # ---- Test 2: aggregate (suffix=_micro) ----
        agg_path = write_aggregate_stats(sample_dir, suffix="_micro")
        assert agg_path is not None, "aggregate returned None"
        assert os.path.exists(agg_path), f"Missing {agg_path}"
        print("PASS: test_aggregate_micro_stats")
        ...
        # Expected micro segments: [0, 2, 4]
        #   radii: [1.0, 1.5, 2.0] -> avg_radius=1.5, avg_diameter=6.0
        #   lengths: [10, 15, 12] -> avg=12.333...
        #   torts: [1.1, 1.3, 1.4] -> avg=1.2666...
        #   density: 3/0.078 = 38.4615...
        with open(
            os.path.join(
                sample_dir, "run_20240601_120000", "statistics_summary_micro.txt"
            ),
            "r",
            encoding="utf-8",
        ) as f:
