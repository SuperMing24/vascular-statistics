...
class ManualCutGeometryTests(unittest.TestCase):
    ...
    def test_retained_volume_uses_union_across_layers(self):
        first = layer("p1", [[0.5, 0.5], [2.5, 0.5], [2.5, 2.5], [0.5, 2.5]], 1, 1)
        second = layer("p2", [[1.5, 0.5], [2.5, 0.5], [2.5, 2.5], [1.5, 2.5]], 1, 2)
        result = compute_retention((2, 3, 3), [first, second])
        self.assertEqual(result.total_voxels, 18)
        self.assertEqual(result.deleted_voxels, 15)
        self.assertEqual(result.retained_voxels, 3)

    def test_pajek_cut_removes_incident_edges_and_relabels(self):
        graph = nx.Graph()
        for node, x in enumerate((0.0, 1.0, 2.0)):
            graph.add_node(str(node), pos=f"[{x} 0 0]", r=str(node + 1), type="2")
        graph.add_edges_from([("0", "1"), ("1", "2")])
        bad = layer("p1", [[0.5, 0.5], [1.0, 0.5], [1.0, 1.0], [0.5, 1.0]])

        with tempfile.TemporaryDirectory() as tmp:
            source = os.path.join(tmp, "source.pajek")
            output = os.path.join(tmp, "output.pajek")
            nx.write_pajek(graph, source)
            result = cut_pajek_graph(source, [bad], output)
            cut_graph = nx.read_pajek(output)

        self.assertEqual(result.deleted_nodes, 1)
        self.assertEqual(result.deleted_edges, 1)
        self.assertEqual(sorted(cut_graph.nodes()), ["0", "1"])
        self.assertEqual(cut_graph.number_of_edges(), 1)
